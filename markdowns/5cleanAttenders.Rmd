...
```{r makeClass2ZeroesNAs}
# Make '0' into NA for Class 2 ---------------------
attenders %<>%
  mutate(class2 = replace(Class2, Class2 == 0, NA))

# Drop capitalised Class2 ---------------------------
...
attenders %<>%
  mutate(ethnicity =
           replace(Ethnicity, Ethnicity == 0, NA) %>% 
           factor %>% 
           fct_recode("white" = "1",
                      "black" = "2",
                      "asian" = "3",
...
Just realised that `~ replace(.,.==0,NA) %>% factor %>% fct_recode %>% fct_explicit_na()` is equivalent to `~factor(.) %>% fct_recode("(Missing)" = "0")`